    ...
    try {
      ...
      if (existing) {
        ...
        await (supabase as any)
          .from("qr_codes")
          .insert({ menu_item_id: id, restaurant_id: restaurantId, qr_data_url: qr });
      }
  ...
  const inp = (k: string, label: string, extra: any = {}) => (
    <div>
      ...
      <Input
        ...
        className="mt-1 bg-input focus-visible:ring-primary h-11"
        {...extra}
      />
    </div>
  );
    ...
    <form onSubmit={submit} className="space-y-6 max-w-3xl">
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <div>
          ...
          <Textarea
            ...
            className="mt-1 bg-input focus-visible:ring-primary"
          />
        </div>
          ...
          <div>
            ...
            <div className="mt-1 relative">
              ...
              <Input
                ...
                className="pl-7 bg-input focus-visible:ring-primary h-11"
              />
            </div>
          </div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {SPICE_LEVELS.map((s) => (
              <button
                type="button"
                key={s.value}
                onClick={() => set("spice_level", s.value)}
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] uppercase tracking-wider px-2 py-1 rounded-full border border-primary/30 text-primary/80">
            Internal — MenuVerse team
          </span>
        </div>
        <p className="text-xs text-muted-foreground">
          Restaurant clients should send dish photos to the MenuVerse team — these fields are
          prepared and populated internally.
        </p>
      ...
      <Button
        type="submit"
        disabled={saving}
        className="bg-primary text-primary-foreground gold-glow rounded-xl h-12 px-8"
      >
