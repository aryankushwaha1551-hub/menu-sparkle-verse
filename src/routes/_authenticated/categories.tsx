...
function Categories() {
  ...
  const add = async (n: string, em: string) => {
    ...
    const { error } = await (supabase as any)
      .from("categories")
      .insert({ restaurant_id: restaurantId, name: n.trim(), emoji: em || "🍽️" });
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <form
        ...
        className="glass rounded-2xl p-4 flex flex-wrap gap-3 items-end"
      >
        <div className="w-20">
          ...
          <Input
            ...
            className="text-center text-lg h-11 bg-input"
          />
        </div>
        <div className="flex-1 min-w-[200px]">
          ...
          <Input
            ...
            className="h-11 bg-input focus-visible:ring-primary"
          />
        </div>
        <Button type="submit" className="bg-primary text-primary-foreground h-11 rounded-xl">
          Add
        </Button>
      </form>
          ...
          <div className="flex flex-wrap gap-2">
            {SUGGESTIONS.map((s) => (
              <button
                ...
                className="glass rounded-xl px-4 py-2 text-sm hover:border-primary/60 transition-colors"
              >
                {s.emoji} {s.name}
              </button>
            ))}
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {list.map((c) => (
          <div key={c.id} className="glass rounded-2xl p-4 flex items-center justify-between">
            ...
            <button
              onClick={() => del(c.id)}
              className="text-muted-foreground hover:text-destructive p-2 rounded-lg hover:bg-secondary"
            >
