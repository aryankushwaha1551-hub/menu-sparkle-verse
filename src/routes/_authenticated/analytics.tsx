...
function Analytics() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!r) return;
      const { data: scans, count } = await (supabase as any)
        .from("scans")
        .select("menu_item_id, device_type", { count: "exact" })
      ...
      (scans ?? []).forEach((s: any) => {
        grouped[s.menu_item_id] = (grouped[s.menu_item_id] ?? 0) + 1;
      });
      ...
      if (ids.length) {
        ...
        setTop(
          (items ?? [])
            .map((i: any) => ({ ...i, count: grouped[i.id] }))
  ...
  const mp = Math.round((mobile / totalDev) * 100),
    dp = 100 - mp;
      ...
      <div className="glass rounded-2xl p-6 space-y-4">
        ...
        <div>
          ...
          <div className="h-2 rounded-full bg-secondary overflow-hidden">
            <div className="h-full gold-gradient" style={{ width: `${mp}%` }} />
          </div>
        </div>
        <div>
          <div className="flex justify-between text-sm mb-1">
          ...
          <div className="h-2 rounded-full bg-secondary overflow-hidden">
            <div className="h-full gold-gradient" style={{ width: `${dp}%` }} />
          </div>
        </div>
      </div>
    </div>
