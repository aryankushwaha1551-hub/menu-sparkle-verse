...
function QRPage() {
  ...
  const regen = async (item: any) => {
    ...
    if (existing) {
      ...
      await (supabase as any)
        .from("qr_codes")
        .insert({ menu_item_id: item.id, restaurant_id: r.id, qr_data_url: dataUrl });
    }
  ...
  const copy = (slug: string) => {
    navigator.clipboard.writeText(viewerUrl(slug));
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Download, share, or regenerate QR codes for every dish.
        </p>
      </div>
      <Input
        value={q}
        ...
        className="max-w-md bg-input focus-visible:ring-primary"
      />
      <div className="glass rounded-2xl overflow-hidden">
        <div className="divide-y divide-border/40">
          {filtered.length === 0 && (
            <div className="p-10 text-center text-muted-foreground">
              No QR codes yet. Add a dish to generate one.
            </div>
          )}
          {filtered.map((item) => {
              ...
              <div key={item.id} className="p-4 flex flex-wrap items-center gap-4">
                ...
                <div className="h-16 w-16 rounded-lg bg-white p-1 shrink-0">
                  {qr ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-[10px] text-black">
                      No QR
                    </div>
                  )}
                </div>
                <div className="flex gap-2">
                  {qr && (
                    <a href={qr} download={`${item.slug}.png`}>
                      <Button variant="outline" size="sm" className="border-primary/30 rounded-lg">
                  ...
                  <Button
                    ...
                    className="border-primary/30 rounded-lg"
                  >
