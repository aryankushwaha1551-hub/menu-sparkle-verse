...
function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRestaurant(r);
      if (r) {
        const [{ count: dishes }, { count: qr }, { count: scans }, { count: featured }, { data: recentData }] =
          await Promise.all([
            (supabase as any).from("menu_items").select("*", { count: "exact", head: true }).eq("restaurant_id", r.id),
            (supabase as any).from("qr_codes").select("*", { count: "exact", head: true }).eq("restaurant_id", r.id),
            (supabase as any).from("scans").select("*", { count: "exact", head: true }).eq("restaurant_id", r.id),
            (supabase as any).from("menu_items").select("*", { count: "exact", head: true }).eq("restaurant_id", r.id).eq("is_featured", true),
            (supabase as any).from("menu_items").select("*").eq("restaurant_id", r.id).order("created_at", { ascending: false }).limit(6),
          ]);
