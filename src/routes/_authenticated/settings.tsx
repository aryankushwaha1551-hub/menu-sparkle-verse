...
function Settings() {
  ...
  const save = async (e: React.FormEvent) => {
    ...
    const { error } = await (supabase as any).from("restaurants").update({
      name: r.name, logo_url: r.logo_url, phone: r.phone, email: r.email,
      address: r.address, instagram: r.instagram, website: r.website, opening_hours: r.opening_hours,
    }).eq("id", r.id);
    setSaving(false);
  ...
  const field = (k: string, label: string, extra: any = {}) => (
    <div>
      ...
      <Input value={r[k] ?? ""} onChange={(e) => setR({ ...r, [k]: e.target.value })}
        className="mt-1 bg-input border-border focus-visible:ring-primary h-11" {...extra} />
    </div>
  );
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <form onSubmit={save} className="glass rounded-2xl p-6 space-y-4">
        ...
        <div>
          ...
          <Textarea value={r.address ?? ""} onChange={(e) => setR({ ...r, address: e.target.value })}
        ...
        <Button disabled={saving} className="bg-primary text-primary-foreground gold-glow rounded-xl h-11 px-8">
