  ...
  if (loading)
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="shimmer h-40 w-80 rounded-2xl" />
      </div>
    );
  if (!dish)
    return (
    ...
    <div className="min-h-screen bg-background text-foreground">
      <div className="max-w-lg mx-auto px-5 pt-6 pb-24">
        ...
        <div className="mt-3 flex flex-wrap gap-2 text-xs">
          <span
            className={`px-2 py-1 rounded-full border ${dish.is_veg ? "border-green-500/40 text-green-400" : "border-red-500/40 text-red-400"}`}
          >
            {dish.is_veg ? "🌿 Veg" : "🍖 Non-Veg"}
          </span>
          <span className="px-2 py-1 rounded-full border border-primary/30">
            {s.emoji} {s.label}
          </span>
          {dish.prep_time && (
            <span className="px-2 py-1 rounded-full border border-border">⏱ {dish.prep_time}</span>
          )}
        </div>

        <div className="mt-4 text-3xl font-bold">{formatRupees(dish.price)}</div>
        {dish.description && (
          <p className="mt-3 text-muted-foreground leading-relaxed">{dish.description}</p>
        )}
              ...
              {dish.thumbnail_url ? (
                ...
                <div>
                  <div className="font-medium text-foreground">
                    This interactive dish experience is being prepared.
                  </div>
                  <div className="text-sm mt-1">
                    Explore other signature dishes from {restaurant?.name ?? "this restaurant"}.
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <a
            href={whatsappHref(restaurant?.phone, `I want to order ${dish.name}`)}
            target="_blank"
          ...
          <Button
            variant="outline"
            onClick={share}
            className="h-12 rounded-xl border-primary/30 hover:bg-primary/10"
          >
